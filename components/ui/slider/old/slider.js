 ...
 */
typeof include === "function" && include("control/input.js, typeof include === "function" && include("dom/drag.js, ");));
/**
");
 * ��ʾһ�����顣
 * @class
...
var Slider = Input.extend({
    ...
    onClick: function (e) {
        ...
        var handles = this.elem.queryAll('.x-slider-handle'), left = e.pageX - this.elem.getRect().left, leftP = left * 100 / this.elem.offsetWidth, min = 101, handle = handles[0];
        // �ҵ������ĵ㡣
    ...
    _handleDragStart: function (handle, draggable) {
        ...
        var handles = this.elem.queryAll('.x-slider-handle'), min = 101, max = -1, value;
    ...
    onChange: function () {
        ...
        // ���û���ǰ��ɫ��
        var handles = this.elem.queryAll('.x-slider-handle'), startHandle = this.getStartHandle(), endHandle = this.getEndHandle(), start = startHandle ? parseFloat(startHandle.style.left) : 0, fore = this.elem.query('.x-slider-fore');
    ...
    getStartHandle: function () {
        var handles = this.elem.queryAll('.x-slider-handle'), min = 0;
    ...
    getEndHandle: function () {
        var handles = this.elem.queryAll('.x-slider-handle'), max = 0;
        for (var i = 1; i < handles.length; i++) {
