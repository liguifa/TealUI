        ...
        if (oldContainer) {
            if (animation) {
                ...
                if (/^zoom/.test(animation)) {
                    var container = reverse ? oldContainer : newContainer, actived = container.querySelector('.x-calender-actived'), activedRect = actived.getRect(), tableRect = container.getRect(), origin = (activedRect.left - tableRect.left + activedRect.width / 2) + 'px ' + (activedRect.top - tableRect.top + activedRect.height / 2) + 'px';
                    newFrom = {
                        transform: 'scale(4, 4)',
...
Calender.views = {
    day: {
        ...
        render: function (calender, container, title) {
            ...
            container.html(Calender._generateTable('x-calender-days', 6, 7, function () {
                ...
                if (holiday) {
                    html += ' x-calender-holiday';
                }
                // 超过日期范围。
                if (tmpDateValue < min || tmpDateValue > max) {
                    html += ' x-calender-invalid';
                }
                // 当前选中的日期。
                if (tmpDateValue === value) {
    ...
    month: {
         ...
         */
        render: function (calender, container, title) {
            var displayValue = calender.displayValue, value = this.picker === 'month' && calender.value, year = displayValue.getFullYear();
            // 设置顶部标题。
            ...
            container.innerHTML = Calender._generateTable('x-calender-months', 3, 4, function (month) {
                ...
                // 超过日期范围。
                if (compare(calender.min) < 0 || compare(calender.max) > 0) {
                    html += ' x-calender-invalid';
                }
                // 当前选中的日期。
                if (compare(value) === 0) {
    ...
    year: {
        render: function (calender, container, title) {
            var displayValue = calender.displayValue, value = this.picker === 'year' && calender.value, startYear = Math.floor(displayValue.getFullYear() / 10) * 10 - 1, year = startYear;
            // 设置顶部标题。
    ...
    decade: {
        render: function (calender, container, title) {
            var displayValue = calender.displayValue, value = this.picker === 'year' && calender.value, startYear = Math.floor(displayValue.getFullYear() / 100) * 100 - 10, year = startYear;
            // 设置顶部标题。
