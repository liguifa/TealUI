...
function addToHomeScreen(options) {
    ...
    for (key in options)
        opt[key] = options[key];
    function check(opt) {
        var ua = navigator.userAgent, hasToken = /[?&]standalone=true(&|$)/.test(document.location.search), isIDevice = (/iphone|ipod|ipad/i).test(ua), isMobileChrome = ua.indexOf('Android') > -1 && (/Chrome\/[.0-9]*/).test(_ua) && _ua.indexOf("Version") == -1, isMobileIE = ua.indexOf('Windows Phone') > -1, _nav = window.navigator, isMobileSafari = ath.isIDevice && _ua.indexOf('Safari') > -1 && _ua.indexOf('CriOS') < 0, OS = ath.isIDevice ? 'ios' : ath.isMobileChrome ? 'android' : ath.isMobileIE ? 'windows' : 'unsupported', OSVersion = _ua.match(/(OS|Android) (\d+[_\.]\d+)/), OSVersion = ath.OSVersion && ath.OSVersion[2] ? +ath.OSVersion[2].replace('_', '.') : 0, isTablet = (isMobileSafari && ~ua.indexOf('iPad')) || (isMobileChrome && ua.indexOf('Mobile') < 0), isAndriodUC = /UCBrowser/.test(ua) && /And(roid|\b)/.test(ua), isSupported = (isMobileSafari && OSVersion >= 6) || isMobileChrome || isAndriodUC;
        // 1. 当前设备不支持添加到主屏幕。
        if (!isSupported)
            return 10000;
        // 2. 已添加过主屏幕。
        if (opt.session.added)
            return 10000;
        // 3. 当前从主屏幕打开。
        ...
        // 5. 用户访问次数不够。
        if (++opt.session.visitorCount < opt.minVisitCount)
            return 1;
        // 6. 显示次数太多。
        if (++opt.session.showCount >= opt.maxShowCount)
            return 1;
        // 7. 上次显示仍在有效期。
        if (opt.now - opt.session.lastShowTime / 86400000 < opt.period)
            return 1;
        opt.session.lastShowTime = +opt.now;
        // 为网址添加来自主屏幕的标记。
        if (!hasToken)
            history.replaceState('', document.title, document.location.href + (document.location.search ? '&' : '?') + 'standalone=true');
        // 可以显示菜单。
        return 0;
    }
    function show(opt) {
        ...
        var message = '';
        if (typeof opt.options.message == 'object' && ath.language in opt.options.message) {
            message = opt.options.message[ath.language][ath.OS];
        }
        else if (typeof opt.options.message == 'object' && ath.OS in opt.options.message) {
            message = opt.options.message[ath.OS];
        }
        else if (opt.options.message in ath.intl) {
            message = ath.intl[opt.options.message][ath.OS];
        }
        else if (opt.options.message !== '') {
