...
(function () {
    ...
    var bootjs = (function (window) {
        ...
        function loadContent(url) {
            ...
            var xmlHttp = +"\v1" ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP"), status;
            try {
        ...
        function loadModule(moduleUrl) {
            var requires = [], imports = [], sourceCode = loadContent(moduleUrl).replace(/^(\s*)\/[\/\*]\s*#(\w+)\s+(.*)$/gm, function (all, indent, macro, args) {
                switch (macro) {
                    case 'include':
