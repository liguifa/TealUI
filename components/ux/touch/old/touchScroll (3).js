...
Element.prototype.touchScroll = function (options) {
    var swippable = {
        ...
        onSwipeMove: function (e) {
            var deltaX = this.endX - this.startX, deltaY = this.endY - this.startY;
            // 如果刚开始移动，则判断是否移动一段距离。
            if (session.state === 0) {
                // 拖动范围太小，忽略拖动。
        ...
        onSwipeEnd: function (e) {
            if (this.state === 1) {
                // 确定当前实际需要显示的项。
                var items = me.children(), width = 0, endX = -this.currentX;
    ...
    return this.on('touchstart', function (e) {
        ...
        var session = {
            ...
            touchEnd: function (e) {
                ...
                if (session.state === 1) {
                    // 确定当前实际需要显示的项。
                    var items = me.children(), width = 0, endX = -session.currentX;
                    for (var i = 0; i < items.length; i++) {
                        // 原始宽度。
                        var oldWidth = width;
                        // 计算新宽度。
            ...
            touchMove: function (e) {
                var touch = (e.touches || e.originalEvent.touches)[0], deltaX = touch.clientX - session.startX, deltaY = touch.clientY - session.startY;
                // 如果刚开始移动，则判断是否移动一段距离。
                if (session.state === 0) {
                    // 拖动范围太小，忽略拖动。
                    if (deltaX * deltaX + deltaY * deltaY < options.skip * options.skip) {
...
//# sourceMappingURL=touchScroll (3).js.map