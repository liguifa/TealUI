define(["require", "exports"], function (require, exports) {
     ...
     */
    function countDown(end, callback, now = new Date()) {
     ...
     */
    function countUp(start, callback, now = new Date()) {
    ...
    function count(start, callback, now, years) {
        const offset = new Date() - now + +start;
        const step = () => {
            let total = Math.floor((offset - new Date()) / 1000);
            if (total <= 0) {
                if (!years) {
