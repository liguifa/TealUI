...
var Marquee = Base.extend({
    ...
    _animateToWithoutLoop: function (index, lt) {
        var me = this, oldIndex = me._fixIndex(me.currentIndex), obj;
    ...
    _animateToWithLoop: function (index, lt) {
        var me = this, oldIndex = me._fixIndex(me.currentIndex);
        if (me.beforeChange(index, oldIndex) !== false) {
            // 暂停自动播放，防止出现抢资源问题。
            me.pause();
            Dom.animate(me.elem, {
                ...
                start: function (options) {
                    // 实际所滚动的区域。
                    var actualIndex = index + me.length, prop = me._horizonal ? 'marginLeft' : 'marginTop', from = Dom.styleNumber(me.elem, prop), to = -me._getScrollByIndex(actualIndex);
                    // 如果保证是平滑滚动，则修正错误的位置。
                    if (me.flow) {
                        // 如果是往上、左方向滚。
                        if (lt) {
     ...
     */
    update: function () {
        var children = Dom.children(this.elem), size, xy = this._horizonal ? 'Width' : 'Height';
        if (!this.cloned) {
            // 设置大小。
