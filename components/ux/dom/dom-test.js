define(["require", "exports", "assert", "./dom"], function (require, exports, assert, dom) {
    ...
    function appendTest() {
        ...
        afterEach();
        beforeEach();
        const old = document.getElementById("sap").innerHTML;
        ...
        let pass = true;
        try {
            const body = document.getElementById("iframe").contentWindow.document.body;
            if (body !== null) {
                pass = false;
    ...
    function getAttrTest() {
        ...
        const $button = dom.after(document.getElementById("button"), "<button value='foobar'>text</button>");
    ...
    function getTextTest() {
        ...
        assert.strictEqual(dom.getText(document.getElementById("foo")), "foo", "Text node was retreived from !.textContent!.");
    ...
    function setHtmlTest() {
        ...
        div = dom.parse("<div/>");
        dom.setHtml(div, "<div id='parent_1'><div id='child_1'/></div><div id='parent_2'/>");
        ...
        var div = dom.parse("<div/>");
        dom.setHtml(div, "&amp;");
        assert.strictEqual(div.innerHTML, "&amp;", "Make sure entities are passed through correctly.");
        ...
        var div = dom.parse("<div />");
        dom.setHtml(div, 5);
        assert.strictEqual(dom.getHtml(div), "5", "Setting a number as html");
        dom.setHtml(div, 0);
    ...
    function setStyleTest() {
        ...
        var div = document.getElementById("nothiddendiv"), display = div.getStyle("display"), ret = div.setStyle("display", '');
    ...
    function setRectTest() {
        ...
        export function getSizeTest() {
            ...
            // setup html
            var $divNormal = dom.parse("<div>").set({ width: "100px", height: "100px", border: "10px solid white", padding: "2px", margin: "3px" }), $divChild = $divNormal.clone(), $divHiddenParent = dom.parse("<div>").set("display", "none").append($divChild).appendTo();
            $divNormal.appendTo();
        ...
        function testoffset(name, fn) {
            test(name, function () {
                ...
                var iframe = loadFixture(), win = iframe.contentWindow, interval = setInterval(function () {
            ...
            function loadFixture() {
                var src = "./data/offset/" + name + ".html?" + parseInt(Math.random() * 1000, 10), iframe = dom.parse("<iframe />").set({
        ...
        if (eval("!-[1,]")) {
            ...
            test("Setting opacity to 1 properly removes filter: style", function () {
                var rfilter = /filter:[^;]*/i, test = document.getElementById("t6652").setStyle("opacity", 1), test2 = test.find("div").setStyle("opacity", 1);
