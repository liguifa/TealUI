define(["require", "exports"], function (require, exports) {
     ...
     */
    function des(value, key, decrypt = false, iv, padding = 0) {
        ...
        for (let p = 0; p < len;) {
            ...
            left ^= t;
            right ^= t << 2;
            t = (right >>> 8 ^ left) & 0x00ff00ff;
            left ^= t;
            right ^= t << 8;
            t = (left >>> 1 ^ right) & 0x55555555;
            right ^= t;
            left ^= t << 1;
            left = left << 1 | left >>> 31;
    ...
    function createKeys(key) {
        ...
        const result = new Array(count * 32);
        let p = 0;
        let ri = 0;
        for (let i = 0; i < count; i++) {
            ...
            right ^= t;
            left ^= t << 4;
            t = (right >>> -16 ^ left) & 0x0000ffff;
            left ^= t;
            right ^= t << -16;
            t = (left >>> 2 ^ right) & 0x33333333;
            right ^= t;
            left ^= t << 2;
            t = (right >>> -16 ^ left) & 0x0000ffff;
            left ^= t;
