...
const Doc = {
        ...
        if (Doc._hasQuery("doc-fullscreen")) {
            ...
            if (meta.test && Doc._hasQuery("doc-unittest")) {
                ...
                Doc._get("doc_main").innerHTML = `<aside id="doc_mainmenu" class="doc-menubutton" style="margin: -.25rem 1rem 0">
                    ...
                    <div style="display: none" class="doc">${Doc._get("doc_main").innerHTML}</div>`;
            ...
            Doc._each(".doc-menubutton>menu a", elem => {
                elem.onclick = () => {
                    const menu = elem.parentNode.parentNode;
                    menu.style.opacity = "0";
                    setTimeout(() => {
                        menu.style.display = "none";
                    }, 300);
                    setTimeout(() => {
                        menu.style.display = menu.style.opacity = "";
                    }, 600);
                };
            });
    ...
    relayout() {
        ...
        Doc._get("doc_sidebar_menu").style.height = innerHeight - Doc._get("doc_sidebar_header").offsetHeight + "px";
        ...
        const append = items => {
            if (Array.isArray(items)) {
                for (const item of items) {
                    ...
                    html += `<li${item.href === active ? ` class="doc-sidebar-active" aria-selected="true"` : ""}><a href="${Doc._meta.root}/${Doc._meta.dir}/${item.href}" title="${title}">${item.titleHtml || title}<small>${item.nameHtml || Doc._encodeHTML(Doc.nameOfItem(item.href))}</small></a></li>`;
                }
            }
            else {
     ...
     */
    copy(content) {
        const textarea = Doc._insert("<textarea></textarea>");
    ...
    writeList() {
        ...
        const addGroup = (data, title) => {
            ...
            for (const key in data) {
                ...
                minSection.html += `<h3 id="${Doc._encodeHTML(key)}">${Doc._encodeHTML(key)}<small>(${items.length || "0"})</small></h3><ul>`;
                if (Array.isArray(items)) {
                    for (const item of items) {
                        const title = Doc._encodeHTML(item.title);
    ...
    _scrollTo(elem, value, delta) {
        ...
        const step = () => {
            if (elem.scrollTop === last) {
                elem.scrollTop = start + (1 - Math.pow((1 - count / 25), 3)) * length;
                if (count++ < 25) {
                    last = elem.scrollTop;
                    setTimeout(step, 20);
                }
